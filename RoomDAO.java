...
public class RoomDAO {
    ...
    public List<Room> getAllRooms() throws SQLException {
        ...
        String query = "SELECT r.*, m.Title, s.ShowDate, m.StartDate, m.EndDate " +
                "FROM Room r " +
            ...
            while (rs.next()) {
                ...
                if (showDate == null || startDate == null || endDate == null) {
                    room.setStatus("Không chiếu");
                        ...
                        currentDate.after(showDate) && currentDate.before(new Date(showDate.getTime() + 2 * 60 * 60 * 1000))) {
    ...
    public List<Seat> getSeatsByRoomId(int roomID) throws SQLException {
        ...
        String query = "SELECT * FROM Seat WHERE RoomID = ?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setInt(1, roomID);
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    Seat seat = new Seat(); // Now works with the no-argument constructor
