    ...
    private void createSeatsForRoom(Room room) throws SQLException {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            for (int i = 0; i < capacity; i++) {
                // Tính hàng dựa trên chỉ số (A=0, B=1, C=2,...)
                char row = (char) ('A' + (i / 10)); // Chia lấy nguyên để xác định hàng
                int seatNumber = (i % 10) + 1; // Lấy số thứ tự trong hàng (1-5)
...
//            for (int i = 0; i < capacity; i++) {
//                // Tính hàng dựa trên chỉ số (A=0, B=1, C=2,...)
//                char row = (char) ('A' + (i / 5)); // Chia lấy nguyên để xác định hàng
//                int seatNumber = (i % 5) + 1; // Lấy số thứ tự trong hàng (1-5)
//                String seatName = row + String.valueOf(seatNumber);
//                stmt.setInt(1, room.getRoomID());
//                stmt.setString(2, seatName);
//                stmt.addBatch();
